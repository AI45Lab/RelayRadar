  ...
  async function runSentinelNow(): Promise<void> {
    ...
    try {
      ...
      setSentinelRunFeedback({
        text: res.ok ? "Sentinel check completed for this endpoint." : (res.message ?? "Sentinel check did not run."),
        isError: false
      });
    } catch (err) {
      setSentinelRunFeedback({
      ...
      setBaselineFeedback({
        text:
          baselineDraftId.trim().length > 0
            ? "Baseline selection updated for this endpoint."
    ...
    <section>
      <div className="section-header">
        <div>
          ...
          <p className="muted small">Global Sentinel policy applies to all endpoints, not just this one.</p>
        </div>
      ...
      {detail.loading ? <p>Loading endpoint details...</p> : null}
          ...
          <article className="card endpoint-hero">
            <div className="endpoint-hero-top">
              <div className="endpoint-hero-title">
                <h3>{detail.data.endpointName}</h3>
